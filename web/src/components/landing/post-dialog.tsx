...
export default function PostDialog({ open, onOpenChange, post }: PostDialogProps) {
  if (!post) return null
          ...
          <article className="relative rounded-[24px] border-[3px] border-foreground bg-card text-card-foreground shadow-[0_6px_0_0_var(--color-foreground)] overflow-hidden">
            <header className="px-8 pt-10 pb-4 text-center">
              <DialogHeader className="p-0">
                <DialogTitle className="text-balance text-3xl md:text-4xl font-extrabold uppercase tracking-wide">
                  {post.title}
                </DialogTitle>
              </DialogHeader>
            </header>
            ...
            <figure className="w-full border-b-[3px] border-foreground">
              <img
                src={post.image || "/placeholder.svg?height=480&width=1200&query=full%20width%20post%20hero"}
            ...
            <div className="px-10 pt-6 pb-16">
              <p className="text-pretty leading-relaxed text-foreground/80">{post.body}</p>
            </div>
